-- =====================================================
-- CORRECT SCHEMA MIGRATION - MATCHES EXISTING STRUCTURE
-- Based on codebase analysis and existing schema
-- =====================================================

-- 1. CREATE ENDORSEMENTS TABLE (matching existing schema)
DROP TABLE IF EXISTS public.endorsements CASCADE;
CREATE TABLE public.endorsements (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    veteran_id uuid NOT NULL, -- matches existing schema
    endorser_id uuid NOT NULL, -- matches existing schema
    text text, -- matches existing schema
    created_at timestamptz DEFAULT now()
);

-- 2. CREATE LIKES TABLE (for pitch likes)
...
-- 3. CREATE SHARES TABLE (for pitch shares)
...
-- 11. CREATE FOREIGN KEY RELATIONSHIPS (if pitches table exists)
DO $$
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'pitches') THEN
        ...
        ALTER TABLE public.likes ADD CONSTRAINT fk_likes_pitch_id FOREIGN KEY (pitch_id) REFERENCES public.pitches(id) ON DELETE CASCADE;
        ALTER TABLE public.shares ADD CONSTRAINT fk_shares_pitch_id FOREIGN KEY (pitch_id) REFERENCES public.pitches(id) ON DELETE CASCADE;
        
        RAISE NOTICE 'Foreign key relationships created with pitches table';
    ELSE
        RAISE NOTICE 'Pitches table not found - skipping foreign key constraints';
    END IF;
END $$;
...
-- SUCCESS!
SELECT 'Migration completed successfully! Schema matches existing structure.' as status;
