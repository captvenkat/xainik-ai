-- Corrected Schema Synchronization Fix
-- This migration fixes all the issues identified in the recheck
-- Handles column existence checks to avoid errors
...
DO $$
BEGIN
    -- Check if user_id column exists and veteran_id doesn't exist
    IF EXISTS (
...
DO $$
BEGIN
    -- Add pitches constraint if veteran_id column exists
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_schema = 'public' 
        AND table_name = 'pitches' 
    ...
    -- Add endorsements constraints if columns exist
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_schema = 'public' 
        AND table_name = 'endorsements' 
...
-- 5. Create indexes for performance (with existence checks)
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'referrals' AND policyname = 'Supporters can create referrals') THEN
