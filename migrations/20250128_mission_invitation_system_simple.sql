-- =====================================================
-- SIMPLIFIED MISSION INVITATION SYSTEM
-- Generic invitations - no role selection complexity
-- =====================================================

-- Mission Invitations Table (Simplified)
CREATE TABLE IF NOT EXISTS mission_invitations (
  ...
  invitation_message TEXT DEFAULT 'Join me in supporting veterans'' success!',
  invitation_link text NOT NULL UNIQUE,
  status VARCHAR(20) DEFAULT 'pending' CHECK (status IN ('pending', 'accepted', 'declined', 'expired')),
  accepted_at timestamptz,
  accepted_user_id uuid REFERENCES users(id) ON DELETE SET NULL,
  accepted_user_role VARCHAR(50) CHECK (accepted_user_role IN ('veteran', 'recruiter', 'supporter')),
...
-- Mission Invitation Analytics Table (Simplified)
...
SELECT 
  ...
  COUNT(*) FILTER (WHERE mi.accepted_user_id IS NOT NULL AND mi.accepted_user_role = 'veteran') as veteran_registrations,
  COUNT(*) FILTER (WHERE mi.accepted_user_id IS NOT NULL AND mi.accepted_user_role = 'recruiter') as recruiter_registrations,
  COUNT(*) FILTER (WHERE mi.accepted_user_id IS NOT NULL AND mi.accepted_user_role = 'supporter') as supporter_registrations,
...
-- Function to create mission invitation (Simplified)
CREATE OR REPLACE FUNCTION create_mission_invitation(
  p_inviter_id uuid,
  p_inviter_role text,
  p_invitee_email text DEFAULT NULL,
  p_invitee_name text DEFAULT NULL,
  p_invitation_message text DEFAULT 'Join me in supporting veterans'' success!',
...
BEGIN
  ...
  INSERT INTO mission_invitations (
    ...
    invitation_message, invitation_link, platform
  ) VALUES (
    p_inviter_id, p_inviter_role, p_invitee_email, p_invitee_name,
    p_invitation_message, v_invitation_link, p_platform
...
-- Function to accept mission invitation (Simplified)
...
COMMENT ON TABLE mission_invitations IS 'Simplified mission invitations - no role selection, generic invitations to join the mission';
...
COMMENT ON FUNCTION create_mission_invitation IS 'Create a simplified mission invitation with tracking';
