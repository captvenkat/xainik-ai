...
function VeteranDashboardContent() {
  ...
  async function checkDatabaseStatus() {
    try {
      ...
      // Check if core tables exist by querying a table we know exists
...
function AnalyticsTab({ userId, router, onSharePitch, searchParams }: { userId: string; router: any; onSharePitch: () => void; searchParams: URLSearchParams }) {
  ...
  if (searchParams.get('created') === 'true' && hasPitches) {
    ...
    // Step 1: Complete Profile - Simple, focused approach
    return (
      <div className="text-center py-12">
    ...
    // Step 2: Create Pitch - Simple, focused approach
    return (
      <div className="text-center py-12">
        <div className="max-w-md mx-auto">
          <div className="text-6xl mb-6">🚀</div>
        ...
        <div className="bg-gradient-to-r from-green-50 to-blue-50 rounded-xl p-6 border border-green-200 shadow-lg">
          <div className="text-center">
            ...
            <button
              ...
              className="bg-gradient-to-r from-blue-500 to-indigo-500 text-white px-8 py-3 rounded-lg hover:from-blue-600 hover:to-indigo-600 transition-all duration-200 text-lg font-semibold shadow-lg hover:shadow-xl transform hover:scale-105 flex items-center gap-2 mx-auto"
            >
      ...
      {userPitchId && (
        <div className="space-y-6">
          ...
          {searchParams.get('created') === 'true' && (
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 border border-blue-200 shadow-lg">
              <div className="text-center mb-4">
                <h3 className="text-xl font-bold text-gray-900 mb-2">Ready to Maximize Your Reach?</h3>
                <p className="text-gray-600">Use AI-powered smart sharing to get discovered by the right people</p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="text-center p-4 bg-white rounded-lg border border-blue-100">
                  <div className="text-2xl mb-2">📱</div>
                  <h4 className="font-semibold text-gray-900">Smart Share</h4>
                  <p className="text-sm text-gray-600">AI-optimized sharing across platforms</p>
                </div>
                <div className="text-center p-4 bg-white rounded-lg border border-blue-100">
                  <div className="text-2xl mb-2">🎯</div>
                  <h4 className="font-semibold text-gray-900">Target Audience</h4>
                  <p className="text-sm text-gray-600">Reach the right recruiters and supporters</p>
                </div>
                <div className="text-center p-4 bg-white rounded-lg border border-blue-100">
                  <div className="text-2xl mb-2">📊</div>
                  <h4 className="font-semibold text-gray-900">Track Results</h4>
                  <p className="text-sm text-gray-600">Monitor views, shares, and engagement</p>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <AIContactSuggestions userId={userId} />
...
function PitchesTab({ userId, router }: { userId: string; router: any }) {
  ...
  const fetchPitches = useCallback(async () => {
    try {
      ...
      // Check if core tables exist by querying a table we know exists
      const { error: coreTableCheck } = await supabase
        .from('pitches')
        .select('id')
        .limit(1)
      
      if (coreTableCheck && (coreTableCheck.message.includes('relation') || coreTableCheck.message.includes('table'))) {
        // Core tables don't exist yet - show error
      ...
      <div className="bg-gradient-to-br from-green-50 to-blue-50 rounded-2xl p-8 text-center shadow-lg border border-green-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white rounded-lg p-6 shadow-sm">
            ...
            <p className="text-sm text-gray-600">Showcase your military expertise and civilian potential</p>
          </div>
          <div className="bg-white rounded-lg p-6 shadow-sm">
            <Users className="w-12 h-12 text-green-600 mx-auto mb-3" />
            <h3 className="font-semibold text-gray-900 mb-2">Network Building</h3>
            <p className="text-sm text-gray-600">Connect with recruiters and supporters who value veterans</p>
          </div>
          <div className="bg-white rounded-lg p-6 shadow-sm">
            <Trophy className="w-12 h-12 text-purple-600 mx-auto mb-3" />
            <h3 className="font-semibold text-gray-900 mb-2">Career Growth</h3>
            <p className="text-sm text-gray-600">Open doors to new opportunities and career advancement</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <p className="text-sm text-gray-500">
            Takes only 5 minutes • AI-powered assistance • Free forever
          </p>
        </div>
      </div>
