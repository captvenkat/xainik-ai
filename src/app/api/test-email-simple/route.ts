...
export async function POST(request: NextRequest) {
  try {
    ...
    // Use the provided email, fallback to ceo@faujnet.com
    const recipientEmail = email || 'ceo@faujnet.com'
    ...
    const { data, error } = await resend.emails.send({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <p><strong>Note:</strong> This email was sent via Resend's verified domain.</p>
...
export async function GET() {
  return NextResponse.json({
    message: 'Simple test email endpoint ready',
