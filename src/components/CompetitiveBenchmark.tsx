  ...
  useEffect(() => {
    ...
    const calculateBenchmarks = () => {
      ...
      const mockData: BenchmarkData[] = [
        {
          category: 'Visibility',
          yourScore: userMetrics?.views || 0,
          averageScore: Math.round((userMetrics?.views || 0) * (0.6 + Math.random() * 0.8)),
          topScore: Math.round((userMetrics?.views || 0) * (1.5 + Math.random() * 1.5)),
          percentile: Math.min(95, Math.max(5, Math.round(((userMetrics?.views || 0) / ((userMetrics?.views || 1) * 2)) * 100))),
          trend: Math.random() > 0.5 ? 'up' : 'down',
          improvement: Math.round(((userMetrics?.views || 0) * 0.1) + (Math.random() * 20)),
          rank: Math.floor(Math.random() * 50) + 1,
          totalCompetitors: 150
        },
        {
          category: 'Engagement',
          yourScore: (userMetrics?.calls || 0) + (userMetrics?.emails || 0),
          averageScore: Math.round(((userMetrics?.calls || 0) + (userMetrics?.emails || 0)) * (0.5 + Math.random() * 1.0)),
          topScore: Math.round(((userMetrics?.calls || 0) + (userMetrics?.emails || 0)) * (2.0 + Math.random() * 2.0)),
          percentile: Math.min(95, Math.max(5, Math.round((((userMetrics?.calls || 0) + (userMetrics?.emails || 0)) / (((userMetrics?.calls || 0) + (userMetrics?.emails || 1)) * 3)) * 100))),
          trend: Math.random() > 0.5 ? 'up' : 'down',
          improvement: Math.round((((userMetrics?.calls || 0) + (userMetrics?.emails || 0)) * 0.15) + (Math.random() * 15)),
          rank: Math.floor(Math.random() * 50) + 1,
          totalCompetitors: 150
        },
        {
          category: 'Conversion Rate',
          yourScore: userMetrics?.conversionRate || 0,
          averageScore: Math.round(((userMetrics?.conversionRate || 0) * (0.7 + Math.random() * 0.6)) * 100) / 100,
          topScore: Math.round(((userMetrics?.conversionRate || 0) * (1.8 + Math.random() * 1.2)) * 100) / 100,
          percentile: Math.min(95, Math.max(5, Math.round(((userMetrics?.conversionRate || 0) / ((userMetrics?.conversionRate || 1) * 2.5)) * 100))),
          trend: Math.random() > 0.5 ? 'up' : 'down',
          improvement: Math.round(((userMetrics?.conversionRate || 0) * 0.2 + Math.random() * 5) * 100) / 100,
          rank: Math.floor(Math.random() * 50) + 1,
          totalCompetitors: 150
        },
        {
          category: 'Network Strength',
          yourScore: userMetrics?.endorsements || 0,
          averageScore: Math.round((userMetrics?.endorsements || 0) * (0.4 + Math.random() * 1.2)),
          topScore: Math.round((userMetrics?.endorsements || 0) * (2.5 + Math.random() * 2.0)),
          percentile: Math.min(95, Math.max(5, Math.round(((userMetrics?.endorsements || 0) / ((userMetrics?.endorsements || 1) * 3.5)) * 100))),
          trend: Math.random() > 0.5 ? 'up' : 'down',
          improvement: Math.round(((userMetrics?.endorsements || 0) * 0.25) + (Math.random() * 8)),
          rank: Math.floor(Math.random() * 50) + 1,
          totalCompetitors: 150
        },
        {
          category: 'Content Virality',
          yourScore: userMetrics?.shares || 0,
          averageScore: Math.round((userMetrics?.shares || 0) * (0.3 + Math.random() * 1.4)),
          topScore: Math.round((userMetrics?.shares || 0) * (3.0 + Math.random() * 2.5)),
          percentile: Math.min(95, Math.max(5, Math.round(((userMetrics?.shares || 0) / ((userMetrics?.shares || 1) * 4.5)) * 100))),
          trend: Math.random() > 0.5 ? 'up' : 'down',
          improvement: Math.round(((userMetrics?.shares || 0) * 0.3) + (Math.random() * 12)),
          rank: Math.floor(Math.random() * 50) + 1,
          totalCompetitors: 150
        }
      ]
