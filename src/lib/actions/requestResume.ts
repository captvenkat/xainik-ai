...
export async function requestResume(pitchId: string, recruiterMessage?: string) {
  ...
  try {
    ...
    const emailHtml = render(ResumeRequestEmail({
      veteranName: pitch.profiles.full_name,
