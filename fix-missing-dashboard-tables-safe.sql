-- Fix Missing Dashboard Tables - SAFE VERSION
-- This script safely adds the missing tables that dashboard components are trying to use
...
-- 3. SAFELY DROP AND RECREATE COMMUNITY SUGGESTIONS SUMMARY VIEW
...
DROP POLICY IF EXISTS "Users can create mission summary" ON public.mission_invitation_summary;
...
-- 8. Add some sample data for testing (only if table is empty)
...
SELECT 
  '00000000-0000-0000-0000-000000000000'::uuid, 
  NULL, 
  'https://xainik.com/invite/sample2', 
  'accepted', 
  'linkedin'
WHERE NOT EXISTS (SELECT 1 FROM public.mission_invitation_summary WHERE invitation_link = 'https://xainik.com/invite/sample2');
