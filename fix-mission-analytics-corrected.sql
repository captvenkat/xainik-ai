-- =====================================================
-- MISSION ANALYTICS FIX - CORRECTED
-- Fixes 406 error for mission_invitation_summary
-- Works with existing table structure
-- =====================================================

-- 1. First, let's check if the mission_invitations table exists and what structure it has
-- We'll use the existing table structure from the migration

-- 2. Create or replace the mission_invitation_summary view
-- This view should work with the existing table structure
...
-- 4. Ensure the mission_invitations table has the correct structure
...
-- 6. Enable RLS on mission_invitations table if not already enabled
...
-- 10. Create mission_invitation_events table if it doesn't exist
...
-- 11. Create indexes for events table
...
-- 14. Grant permissions on events table
DO $$ BEGIN 
  BEGIN 
...
-- 15. Create mission_invitation_analytics table if it doesn't exist
...
-- 16. Create indexes for analytics table
...
-- 17. Enable RLS on analytics table
ALTER TABLE public.mission_invitation_analytics ENABLE ROW LEVEL SECURITY;

-- 18. Create RLS policies for analytics table
DO $$ BEGIN 
...
-- 19. Grant permissions on analytics table
DO $$ BEGIN 
  BEGIN 
...
-- 20. Initialize analytics for existing invitations
...
-- 21. Test the view to make sure it works
SELECT 
